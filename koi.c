...
/*   Updated: 2019/10/24 09:07:02 by eharrag-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_koi_count(t_mlx *mlx)
{
	...
	mlx->new_re = mlx->old_re * mlx->old_re * mlx->old_re - 3
		* mlx->old_re * mlx->old_im * mlx->old_im + mlx->c_re;
...
void	ft_draw_koi(t_mlx *mlx)
{
	...
	while (mlx->y < STR_SIZE)
	{
		...
		{
			mlx->new_re = (mlx->x - STR_SIZE / 2) / (0.3
					* mlx->zoom * STR_SIZE) + mlx->move_x;
			mlx->new_im = (mlx->y - STR_SIZE / 2) / (0.3
